...
# AutoCAD 「坵塊法」網格坡度與坡向自動化計算工具 (Grid Method Slope Calculator)

本專案是一個專為 AutoCAD .NET 平台開發的擴充套件，旨在自動化執行水土保持計畫中「坵塊法」的大範圍地形坡度運算與坡向分析。本工具專注於符合台灣水土保持法規，並提供設計師高效率的分析工作流程。

## 核心特色 (Features)

- 自動化計算坵塊法之坡度坡向：使用者僅需提供計劃地形及計畫範圍即可進行自動化計算。
- 自動生成網格：支援自動化劃分 10m/25m 網格，並依據交點數進行後續坡度及坡向計算。
...
> [!WARNING]
> 從 GitHub 下載 ZIP 的使用者請注意：Windows 會自動封鎖從網路下載的 DLL 檔案，導致 AutoCAD `NETLOAD` 失敗（錯誤代碼 `0x80131515`）。請在載入前先雙擊執行 `UNBLOCK.bat` 以解除鎖定。若使用 `git clone` 方式取得專案則不受此影響。
...
若先前生成之資料因數值運算問題有錯誤時，可自行修正網格四邊之交點數(黃色字)及坡向(粉色字)，並執行本指令。系統將會更新快取資料、重新產生坡度網底，並可選擇覆寫更新畫面中的統計表。
...
坵塊法主要用於有實測地形圖時的坡度分析，藉由量測並計算出特定方格內的平均坡度，作為山坡地範圍劃定、檢討變更及土地可利用限度查定的依據。
